...
- **First Ready-row spec-surface checks** — list caching (`ttlMs` / `cacheScope`). Dual `tools/call` body is done (`_response_text` reads both forms). Documented in [docs/spec-surface.md](docs/spec-surface.md). One row at a time, behind the three false-positive harnesses. Do not start Wait-column rows (Tasks, HTTP-over-stdio, WIF) until the WG ships a wire format.
