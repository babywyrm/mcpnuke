...
Usage:
    ...
    mcpredator --targets http://target:9090 --auth-token $TOKEN --json report.json
"""
...
def main():
    ...
    panel_lines = [
        f"[bold cyan]mcpredator v{__version__}[/bold cyan]  [dim]MCP Red Teaming & Security Scanner[/dim]",
        f"Targets : {len(urls)}",
    ...
    if not urls:
        from mcpredator.k8s.scanner import GLOBAL_K8S_FINDINGS
        if GLOBAL_K8S_FINDINGS:
            console.print(f"\n[bold]── K8s-Only Report ({len(GLOBAL_K8S_FINDINGS)} findings) ──[/bold]")
            from mcpredator.core.constants import SEV_COLOR
