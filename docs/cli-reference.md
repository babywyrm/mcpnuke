...
| Option | Description |
|---|---|
| `--fast` | Fast scan: static checks read the full tool catalog. Invoke probes sample the top 5 security-relevant tools and skip heavy probes (input_sanitization, error_leakage, temporal_consistency, ssrf_probe, sdk_cache_poisoning), cap probe workers at 2. Cuts LLM-backed scan time from ~30min to ~2min. Alias for --coverage 5 on the probe sample. |
| `--coverage N` | Limit invoke probes to the top N most security-relevant tools (by keyword risk score). Static checks still read the full catalog. 0 = probe every tool. --fast is an alias for --coverage 5 on that probe sample. Example: --coverage 20 probes ~20% of a 100-tool server. |
