...
| `list_cache` | HIGH–MEDIUM | SEP-2549 `ttlMs` / `cacheScope` on list results. Silent when the fields are absent. A negative or non-integer TTL, or a cacheScope other than public/private, is MEDIUM. Pages of the same list that disagree on cacheScope (omitted defaults to public) are HIGH — mixed scope lets a shared cache serve the wrong caller. MCP-T16 |
