...
echo ""
echo -e "  ${GREEN}# AI scan (faster Phase 2 with parallel Claude workers)${NC}"
echo "  ./scan --targets http://localhost:9090 --fast --claude --claude-max-tools 25 --claude-phase2-workers 3"
echo ""
echo -e "  ${GREEN}# Deterministic benchmark mode (repeatability-focused)${NC}"
echo "  ./scan --targets http://localhost:9090 --fast --claude --deterministic --verbose"
echo ""
echo -e "  ${GREEN}# Camazotz-style target example${NC}"
echo "  ./scan --targets http://<cluster-node>:30080/mcp --fast --claude --probe-calls 5 --claude-max-tools 8 --claude-phase2-workers 2 --verbose"
echo ""
echo -e "  ${GREEN}# Agentic OAuth/JWT flow example${NC}"
echo "  ./scan --targets https://target.example/mcp --oidc-url https://auth.example/realms/agentic --client-id scanner --client-secret SECRET --oidc-scope \"mcp.read mcp.invoke\" --header \"X-Tenant: blue\" --tls-verify"
echo ""
echo -e "  ${GREEN}# Bring up DVMCP locals then scan all 10 challenges${NC}"
echo "  ./tests/dvmcp_reset.sh --setup-only"
echo "  ./scan --port-range localhost:9001-9010 --fast --deterministic --verbose"
echo ""
